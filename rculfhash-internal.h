...
#ifdef DEBUG
#define dbg_printf(fmt, args...)     printk("[debug rculfhash] " fmt, ## args)
#else
#define dbg_printf(fmt, args...)				\
do {								\
	/* do nothing but check printf format */		\
	if (0)							\
		printk("[debug rculfhash] " fmt, ## args);	\
} while (0)
#endif
...
struct cds_lfht {
	 ...
	 * completion.
	 */
	struct mutex resize_mutex;	/* resize mutex: add/del mutex */
	void *resize_attr;	/* Resize threads attributes */
...
#else
#define poison_free(ptr)	kfree(ptr)
#endif
		...
		unsigned long max_nr_buckets)
{
	...
	ht = kzalloc(cds_lfht_size, GFP_KERNEL);
	BUG_ON(!ht);
