...
{
	if (order == 0) {
		ht->tbl_chunk[0] = kzalloc(ht->min_nr_alloc_buckets *
			sizeof(struct cds_lfht_node), GFP_KERNEL);
		BUG_ON(!(ht->tbl_chunk[0]));
	} else if (order > ht->min_alloc_buckets_order) {
		unsigned long i, len = 1UL << (order - 1 - ht->min_alloc_buckets_order);

		for (i = len; i < 2 * len; i++) {
			ht->tbl_chunk[i] = kzalloc(ht->min_nr_alloc_buckets *
				sizeof(struct cds_lfht_node), GFP_KERNEL);
			BUG_ON(!(ht->tbl_chunk[i]));
		}
	}
